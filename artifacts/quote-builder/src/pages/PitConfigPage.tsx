...
function EditItemModal({ catId, catType, item, mode, allIds, onClose, onSaved }: EditItemModalProps) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    if (idTaken) { setError("This ID is already in use"); return; }
        ...
        <form className="admin-modal-body" onSubmit={handleSave} noValidate>
          ...
          <div className="admin-form-row">
            ...
            <div className="edit-field-group" style={{ flex: 2 }}>
              <label>Name</label>
              <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Line item name" />
            </div>
          </div>
    ...
    <div className="admin-modal-backdrop" onClick={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="admin-modal">
        ...
        <form className="admin-modal-body" onSubmit={handleSave} noValidate>
          {error && <div className="edit-modal-error">{error}</div>}
          <div className="edit-field-group">
            <label>Category ID <span className="edit-modal-optional">(unique slug, e.g. add-on)</span></label>
            <input type="text" value={id} onChange={(e) => setId(e.target.value)} placeholder="add-on" />
          </div>
          <div className="edit-field-group">
            <label>Display Name</label>
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Add-on Services" />
          </div>
          <div className="edit-field-group">
            <label>Item Type</label>
            <select className="admin-select" value={type} onChange={(e) => setType(e.target.value as "duration" | "price")}>
              <option value="duration">Duration-based (hrs × hourly rate)</option>
...
export default function PitConfigPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const res = await fetch(`${API_BASE}/api/admin/pit`, { credentials: "include" });
  ...
  async function handleDeleteItem(catId: string, itemId: string, itemName: string) {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/admin/pit/categories/${catId}/items/${itemId}`, {
        method: "DELETE",
        credentials: "include",
      });
                ...
                <table className="admin-table">
                  <thead>
                    <tr>
                      <th>ID</th>
                      <th>Name</th>
                      {(catType === "duration" || catType === "both") && <th>Duration (hrs)</th>}
                      {(catType === "price" || catType === "both") && <th>Price ($)</th>}
                      <th>Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {currentCat.lineItems.length === 0 && (
                      <tr>
                        <td colSpan={5} className="admin-table-empty">No line items in this category</td>
                      </tr>
                    )}
                    {currentCat.lineItems.map((item) => (
                      <tr key={item.id}>
                        ...
                        {(catType === "duration" || catType === "both") && (
                          <td>{(item.duration ?? 0) > 0 ? `${item.duration} hr${item.duration !== 1 ? "s" : ""}` : "—"}</td>
                        )}
                        {(catType === "price" || catType === "both") && (
                          <td>{(item.price ?? 0) > 0 ? `$${(item.price ?? 0).toFixed(2)}` : "—"}</td>
                        )}
                        <td>
