...
function computeTotal(data: Quote): number {
  ...
  const productPitTotal = computeProductRelatedPitTotal(data.groups, yesNoToggles, optToggles);
  return quoteTotal(data) + pitTotal + productPitTotal;
}

interface UserQuoteRow {
  id: string;
  data: Quote;
...
interface EditDrawerProps {
  row: UserQuoteRow;
  onClose: () => void;
  onSaved: (updated: UserQuoteRow) => void;
}
...
function EditDrawer({ row, onClose, onSaved }: EditDrawerProps) {
  ...
  async function handleSave(e: React.FormEvent) {
    ...
    setError("");
    try {
      const res = await fetch(`${API_BASE}/api/quotes/${row.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        credentials: "include",
        body: JSON.stringify({
          meta: { quoteNumber, oppNumber, companyName, customerName, salesRep, validUntil, notes },
...
/* ── Main Page ───────────────────────────────────────────── */
export default function MyQuoteLibraryPage() {
  const [, setLocation] = useLocation();
  const [quotes, setQuotes] = useState<UserQuoteRow[]>([]);
  ...
  const load = useCallback(async () => {
    ...
    setError("");
    try {
      const [quotesRes, amendRes] = await Promise.all([
        fetch(`${API_BASE}/api/quotes/library`, { credentials: "include" }),
      ...
      const data = (await quotesRes.json()) as { quotes: UserQuoteRow[] };
      setQuotes([...data.quotes].reverse());
      if (amendRes.ok) {
        const ad = (await amendRes.json()) as { amendments: { originalQuoteId: string }[] };
        const counts: Record<string, number> = {};
        for (const a of ad.amendments ?? []) {
  ...
  const filtered = quotes.filter((row) => {
    ...
    if (!s) return true;
    return [row.quoteNumber, row.companyName, row.customerName]
  ...
  return (
    <div className="admin-page">
      <div className="admin-topbar">
        <GlobalNavTrigger />
        <h1 className="admin-page-title">My Quote Library</h1>

        <div className="admin-topbar-right">
          <span className="admin-badge">{quotes.length} quote{quotes.length !== 1 ? "s" : ""}</span>
          <button
        ...
        <div className="admin-toolbar">
          <div className="ql-search-wrap admin-search" style={{ maxWidth: 320 }}>
            ...
            <input
              type="text"
              className="ql-search"
              placeholder="Search quotes, companies, customers…"
              value={search}
